...
def eval(model, test_loader, cfg, output_dir, tb_logger=None, title="", data_type='normal'):
    """
    ...
    with torch.no_grad():
        ...
        for cur_it, batch in enumerate(test_loader):
            ...
            for i in range(cfg["num_networks"]):  # By MC dropout, samples the network output several times(=num_networks) given the same input in order to compute the mean and variance for such given input
                if samples is None:
                    out = stat[1] * model(input) + stat[0] # de-normalization
                    samples = out.tolist()
                else:
                    out = stat[1] * model(input) + stat[0] # de-normalization
                    out = out.tolist()
            ...
            NLL, NLL_without_v_noise = evaluate_with_NLL(mean, var, target.tolist(), dataset_name, data_type=data_type)  # compute the Negative log likelihood with mean, var, target value(label)
...
def ood_eval(pu_model, mc_model, test_loader, output_dir,cfg, tb_logger=None, title="", cur_epoch = None, data_type='ood'):
    """
    ...
    with torch.no_grad():
        ...
        for cur_it, batch in enumerate(test_loader):
            ...
            for i in range(cfg["num_networks"]):  # By MC dropout, samples the network output several times(=num_networks) given the same input in order to compute the mean and variance for such given input
                if samples is None:
                    out = stat[1] * mc_model(input) + stat[0] # de-normalization
                    samples = out.tolist()
                else:
                    out = stat[1] * mc_model(input) + stat[0] # de-normalization
                    out = out.tolist()
...
def pu_eval(model, test_loader, cfg, output_dir, tb_logger=None, title="", data_type='normal'):
    """
...
def eval_de(models, test_loader, cfg, output_dir, tb_logger=None, title="", data_type='normal'):
    NLL_list = None
    RMSE_list = None
    mean_list = None
    variance_list = None
    ...
    with torch.no_grad():
        ...
        for cur_it, batch in enumerate(test_loader):
            ...
            for i in range(cfg["num_networks"]):  # By MC dropout, samples the network output several times(=num_networks) given the same input in order to compute the mean and variance for such given input
                if samples is None:
                    out = stat[1] * models[i](input) + stat[0] # de-normalization
                    samples = out.tolist()
                else:
                    out = stat[1] * models[i](input) + stat[0] # de-normalization
                    out = out.tolist()
...
def pu_eval_with_training_dataset(model, train_loader, cfg, output_dir, tb_logger=None, title="", data_type='normal'):
    """
...
def ood_eval_with_training_datset(pu_model, mc_model, train_loader, output_dir,cfg, tb_logger=None, title="", cur_epoch = None, data_type='ood'):
...
def evaluate_with_NLL(mean, var, label, dataset_name, data_type='normal', v_noise=1):
    import yaml
